...
def load_models(models_path):
    #model_1 = load_model(os.path.join(models_path, model_1_name + '.h5'), custom_objects={'_spatial_transformer_layers': RadNet._spatial_transformer_layers, "tf": tf}, compile=False)
    #model_2 = load_model(os.path.join(models_path, model_2_name + '.h5'), custom_objects={'_spatial_transformer_layers': RadNet._spatial_transformer_layers, "tf": tf}, compile=False)
...
def cross_evaluate_models(models_path, samples_list, static_decalib=False):
    ...
    if (len(images)>100):
        visualize_corrected_projections(images[:100], projections_groundtruth[:100], projections_decalib[:100], projections_1[:100], projections_2[:100])
    else:
