...
def create_decalib_transformation(angle_std_deviation, translation_std_deviation):
    """
    Create a random homogeneous transformation matrix with given standard deviation of angle and translation

    Parameters
    ----------
    angle_std_deviation : float
        Std deviation in degree of random rotation in each angle created from standard normal distribution (* 0.5 for roll angle)
    translation_std_deviation : float
        Std deviation in meter of random translation in each axis created from standard normal distribution
...
def create_random_rotation_matrix(angle_std_dev):
    ...
    roll_angle = np.radians(np.random.uniform(low=-angle_std_dev*0.5, high=angle_std_dev*0.5)) 
    ...
    yaw_angle = np.radians(np.random.uniform(low=-angle_std_dev, high=angle_std_dev))
